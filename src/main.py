...
## OBSERVACIÓN
El cambio que tiene respecto al mainv2.py es el modo en que se hace el split.
En este script se hace split de las imágenes de entrenamiento, validación. Las imágenes usadas en inferencia se ingresan manualmente.
"""
import os
import sys
import yaml
...
from scripts.split_dataset import split
...
while True:
    p = input("¿Requiere etiquetar las imágenes? (s/n): ")
    p = p.strip()
    if (m==1 and p.lower() =="s"):
    ...
    elif (m == 2 and p.lower() == "s") : 
        print("Inicializando Labelme de Inferencia")
        inference_images_dir = os.path.join(BASE_DIR, "inference", "images")
        inference_json_dir = os.path.join(BASE_DIR, "inference", "real_masks", "labelme_json")
        tags_maker(inference_images_dir, inference_json_dir, m)
        print("Etiquetado Finalizado")
        print("="*20)
        break
    else: 
...
if (m == 2): 
    inference_json_dir = os.path.join(BASE_DIR, "inference", "real_masks", "labelme_json")
    save_inference_masks_dir = os.path.join(BASE_DIR, "inference", "outputs", "masks")
    print("Conversión Json -> Masks de Inferencia")
    convert_json(inference_json_dir, save_inference_masks_dir, m)
    print("Conversión Finalizada")
    print("="*20)
...
if (m == 1):
    for d in [train_img_dir, train_mask_dir, val_img_dir, val_mask_dir]: ### <- En caso de que no existan las 4 carpetas de la lista se crean. E.o.c se ignora la linea
        os.makedirs(d, exist_ok=True)

    split(images_dir, train_img_dir, train_mask_dir, val_img_dir, val_mask_dir, train_proportion)
