...
def split_train_inference(images_dir, train_img_dir, train_mask_dir, val_img_dir, val_mask_dir, inference_img_dir, inference_mask_dir, train_proportion, inference_proportion):
    images = sorted([
...
# if (condicion): lista.append(x)
    ...
    n_inference = int(len(images)*inference_proportion)
    inference_images = images[:n_inference]
    res_images = images[n_inference:]


    n_train = int(len(res_images) * train_proportion)
    train_images = res_images[:n_train]
    val_images = res_images[n_train:]


    #Ambos ciclos for sirven para copiar el par (imagen,máscara) en las carpetas correspondientes.
    for f in inference_images:
        copy_pair(f,inference_img_dir, inference_mask_dir)
    ...
    print("inference images:", len(os.listdir(f"{BASE_DIR}/inference/images")))
    print("inference masks:", len(os.listdir(f"{BASE_DIR}/inference/real_masks/masks_labels")))
    ...
    train_msks = sorted(os.listdir(f"{BASE_DIR}/dataset/splits/train/masks"))
    assert train_imgs == train_msks, "nombres no calzan en train"


    val_imgs = sorted(os.listdir(f"{BASE_DIR}/dataset/splits/val/images"))
    val_msks = sorted(os.listdir(f"{BASE_DIR}/dataset/splits/val/masks"))
